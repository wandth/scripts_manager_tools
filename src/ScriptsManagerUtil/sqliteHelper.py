        ...
        for root_child in root_dir.entryInfoList(QDir.NoSymLinks | QDir.NoDotAndDotDot | QDir.Files | QDir.Dirs) :  # type: QFileInfo
            ...
            if root_child.isDir() and root_child.baseName() in self.default_tags.keys() :
                for child in QDir(root_child.absoluteFilePath()).entryInfoList(QDir.NoSymLinks | QDir.NoDotAndDotDot | QDir.Files | QDir.Dirs) :  # type: QFileInfo
                    if child.isDir() :
                        new_folder = QDir(child.absoluteFilePath())
                        ...
                        if len(child_files) != 2 and "scripts" not in [x.fileName() for x in child_files] :
                            cmds.error("{} is not a valid script folder".format(child.absoluteFilePath()))
                            continue
                        ...
                        self.__updateScript(name = script_infos[0].baseName(),
                                            ...
                                            script_type = "python", python_type = "module", tag = root_child.baseName())
                    elif child.isFile() :
                        if child.completeSuffix() == "py" :
                            self.__updateScript(name = child.baseName(), script_path = child.absoluteFilePath(), script_type = "python", python_type = "script",
                                                tag = root_child.baseName())
                        elif child.completeSuffix() == "mel" :
                            self.__updateScript(name = child.baseName(), script_path = child.absoluteFilePath(), script_type = "mel", python_type = "",
                                                tag = root_child.baseName())
            elif root_child.isDir() and root_child.baseName() not in self.default_tags.keys() :
                pass
            if root_child.isFile() :
                if root_child.completeSuffix() == "py" :
                    self.__updateScript(name = root_child.baseName(), script_path = root_child.absoluteFilePath(), script_type = "python", python_type = "script", tag = "")
                elif root_child.completeSuffix() == "mel" :
                    self.__updateScript(name = root_child.baseName(), script_path = root_child.absoluteFilePath(), script_type = "mel", python_type = "", tag = "")
